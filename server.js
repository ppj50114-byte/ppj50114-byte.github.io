...
app.post('/login',(req,res)=>{
  ...
  if(!/^[\u4e00-\u9fa5]{2,}$/.test(name)) return res.json({success:false,msg:'姓名须为中文且至少2个字'});
  if(name===ADMIN_USER){ if(password===ADMIN_PASS) return res.json({success:true,role:'admin'}); else return res.json({success:false,msg:'管理员密码错误'}); }
...
app.post('/news',(req,res)=>{
  ...
  const d=readData();
  const item={id:Date.now(),title,content,image,date:new Date().toISOString(),likes:0,likers:[]};
  d.news.unshift(item); saveData(d); broadcastUpdate(); res.json({success:true,item});
});
...
app.post('/upload', upload.single('file'), (req,res)=>{
  ...
  const d=readData();
  const item={id:Date.now(),type,filename:'/uploads/'+req.file.filename,date:new Date().toISOString(),likes:0,likers:[]};
...
app.post('/wish',(req,res)=>{
  ...
  const d=readData();
  const item={id:Date.now(),name,text,date:new Date().toISOString(),likes:0,likers:[]};
...
app.post('/like',(req,res)=>{
  ...
  const d=readData();
  const list=d[type]; if(!list) return res.status(400).json({success:false});
  const item=list.find(i=>String(i.id)===String(id)); if(!item) return res.status(404).json({success:false});
  item.likers=item.likers||[];
  if(!item.likers.includes(name)){ item.likers.push(name); item.likes=(item.likes||0)+1; }
