...
function readStatsMonth(ym){ const p = statsPathForMonth(ym); if(!fs.existsSync(p)) return []; return JSON.parse(fs.readFileSync(p)); }
...
app.post('/login', (req, res)=>{
  ...
  if(!name) return res.json({ success:false, msg:'请输入名字' });
  if(name === ADMIN_USER){
    if(password === ADMIN_PASS){ addStat({type:'login', user:name, role:'admin'}); return res.json({success:true, role:'admin', name}); }
    return res.json({success:false, msg:'管理员密码错误'});
  }
  addStat({type:'login', user:name, role:'customer'});
...
app.post('/news',(req,res)=>{
  ...
  d.news.unshift(item); saveData(d);
  addStat({type:'publish', user: author||'管理员', contentType:'news', contentId:item.id, title:item.title});
  broadcast(); res.json({success:true, item});
});
...
app.post('/like',(req,res)=>{
  ...
  saveData(d); broadcast(); res.json({success:true, likes: item.likes||0, likers: item.likers});
});
...
app.post('/comment',(req,res)=>{
  ...
  broadcast(); res.json({success:true, comment});
});
...
app.post('/reply',(req,res)=>{
  ...
  broadcast(); res.json({success:true, reply});
});
...
app.post('/likeComment',(req,res)=>{
  ...
  saveData(d); broadcast(); res.json({success:true, likes: comment.likes, likers: comment.likers});
});
